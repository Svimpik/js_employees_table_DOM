...
function sortTable(index, ascending) {
    ...
    const sortedRows = rows.sort((a, b)=>{
        ...
        else if (aText < bText) return ascending ? -1 : 1;
        else return 0;
    });
...
tableBody.addEventListener("blur", // eslint-disable-next-line no-shadow
(event)=>{
    const cell = event.target.closest("td");
    if (!cell || !cell.querySelector("input")) return;
    const input = cell.querySelector("input");
    const newValue = input.value.trim();
    if (newValue === "") cell.textContent = originalValue;
    else cell.textContent = newValue;
}, true);
tableBody.addEventListener("keydown", // eslint-disable-next-line no-shadow
(event)=>{
    if (event.key === "Enter") {
        const cell = event.target.closest("td");
        if (!cell || !cell.querySelector("input")) return;
